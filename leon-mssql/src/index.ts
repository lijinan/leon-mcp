...
import { SqlConnectionManager, DatabaseConfig } from './SqlConnectionManager.js';
    ...
    try {
      switch (name) {
        case 'configure_connection':
          if (!args) throw new Error('Arguments required for configure_connection');
          ...
          const configResult = await connectionManager.configureConnection(args as DatabaseConfig);
          return {
            content: [{
              type: 'text',
              text: configResult
        ...
        case 'execute_query':
          if (!args || typeof args.query !== 'string') throw new Error('Query argument required for execute_query');
          const queryResult = await queryExecutor.executeQuery(args.query, args.parameters as Record<string, any> | undefined);
          return {
            content: [{
              type: 'text',
              text: queryResult
        ...
        case 'execute_stored_procedure':
          if (!args || typeof args.procedureName !== 'string') throw new Error('Procedure name required for execute_stored_procedure');
          const procResult = await queryExecutor.executeStoredProcedure(
            args.procedureName, 
            args.parameters as Record<string, any> | undefined
          );
          return {
            content: [{
              type: 'text',
        ...
        case 'get_table_schema':
          if (!args || typeof args.tableName !== 'string') throw new Error('Table name required for get_table_schema');
          const schemaResult = await queryExecutor.getTableSchema(
            args.tableName, 
            typeof args.schema === 'string' ? args.schema : undefined
          );
          return {
            content: [{
              type: 'text',
